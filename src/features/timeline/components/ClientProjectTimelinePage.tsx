...
type ClientProjectTimelinePageProps = {
    data: Awaited<ReturnType<typeof getClientProjectTimeline>>;
};

export function ClientProjectTimelinePage({
    data,
}: ClientProjectTimelinePageProps) {
    return (
                ...
                <div className="flex flex-col gap-8">
                    <header className="rounded-2xl border border-[color:var(--color-border-subtle)] bg-[var(--color-surface-default)] p-5 md:p-8">
                        ...
                        <p className="mt-3 max-w-[760px] text-body text-[color:var(--color-text-muted)]">
                            Les événements affichés ici sont les éléments publiés pour ton
                            suivi projet.
                        </p>
                    </header>
...
function TimelineList({
    ...
    events: ClientProjectTimelinePageProps["data"]["events"];
}) {
    if (events.length === 0) {
        return <EmptyText>Aucun événement partagé pour le moment.</EmptyText>;
    }
...
function NotificationList({
    ...
    notifications: ClientProjectTimelinePageProps["data"]["notifications"];
}) {
    if (notifications.length === 0) {
        return <EmptyText>Aucune notification pour ce projet.</EmptyText>;
    }
