...
type OffersOrientationSectionProps = {
    content: OffersPageContent["orientation"];
};

/**
 * Orientation : aider à repérer sa situation avant de comparer les offres.
 * Les questions de départ posées en index, puis le rappel rassurant que
 * le bon cadre se confirme après un premier échange.
 */
export function OffersOrientationSection({
    content,
}: OffersOrientationSectionProps) {
    return (
        <Section spacing="lg">
            <div className="grid gap-8 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1fr)] lg:gap-14">
                <div className="flex flex-col gap-4">
                    ...
                    <div className="mt-2 flex items-start gap-3 rounded-panel border border-[color:var(--color-decor-gold)]/40 bg-[image:var(--gradient-hero)] bg-[var(--color-surface-default)] p-5">
                        <span
